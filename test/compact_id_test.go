...
func TestGetLastId_CompactScenario(t *testing.T) {
	...
	id1 := app.GetLastId(&backupTodos)
	if id1 != 11 {
		t.Errorf("First summary task ID should be 11, got %d", id1)
	}

	summaryTask1 := app.TodoItem{
		TaskID:   id1,
	...
	id2 := app.GetLastId(&backupTodos)
	if id2 != 12 {
		t.Errorf("Second summary task ID should be 12, got %d", id2)
	}

	summaryTask2 := app.TodoItem{
		TaskID:   id2,
...
func TestCompactTaskIDGeneration_Integration(t *testing.T) {
	...
	newBackupTodos := make([]app.TodoItem, 0)
	...
	for i := 0; i < 2; i++ {
		id := app.GetLastId(&newBackupTodos)
		summaryTask := app.TodoItem{
			TaskID:   id,
